...
static void UpdateDown(PQItem **arrayOfPQItems, int start, int stop) {
    ...
    while (1) {
        ...
        if (leftChildIndex <= stop &&
            arrayOfPQItems[highestPriorityElementIndex]->nPrior < arrayOfPQItems[leftChildIndex]->nPrior) {
            highestPriorityElementIndex = leftChildIndex;
        }
        // Check if the priority of right side child is EVEN higher
        if (rightChildIndex <= stop &&
            arrayOfPQItems[highestPriorityElementIndex]->nPrior < arrayOfPQItems[rightChildIndex]->nPrior) {
...
int PQsetPrior(PQueue *queue, int priority, int index) {
    ...
    // Check whether the index is not too high
    if (index >= PQSize(queue)) return PRIOR_ERROR;
...
int PQGetPrior(PQueue *queue, PQInfo *info, int (*compareTwoUserInfo)(const void *, const void *)) {
    checkPointer(queue, ERROR_NULL_POINTER_QUEUE);
    checkPointer(info, ERROR_NULL_POINTER_PQINFO);
    checkPointer(compareTwoUserInfo, ERROR_NULL_POINTER_COMPARE_FN);
...
int PQgetPrior(PQueue *queue, int index) {
    ...
    // Ensure there is a queue item on given index
    PQItem *element = queue->pQueue[index];
    checkPointer(element, "Proba znaleziania priorytetu elementu na nieistniejacym indexie");

    return element->nPrior;
}
